...
rules = object _ as rules_object with
  ...
  fragment_until fragment_close =
    .alt
      (.read.text fragment_close =>> .r (.r ()))
      (
        rules_object.fragment_item >>= \fragment_item_writer ->
        rules_object.fragment_until fragment_close >>= \fragment_rest_writer ->
  ...
  choose =
    ...
    .read.text "," =>>
    rules_object.fragment_group "(" ")" >>= \rules_yes_writer ->
    .read.text "," =>>
    rules_object.fragment_group "(" ")" >>= \rules_no_writer ->
    .read.text ")" =>>
    .read.end =>>
  ...
  define =
    ...
    .write.text "=" =>>
    .write.sep =>>
    .write.data rules_object.choose
...
layout = object _ as layout_object with
  ...
  choose =
    ...
    .read.sep =>>
    layout_object.read_option () >>= \layout_yes_writer ->
    layout_object.read_option () >>= \layout_no_writer ->
    .read.end =>>
    if layout_selection == 'yes
      then layout_yes_writer
      else layout_no_writer

  # This macro executes after `layout.choose` and treats the replayed layout as
...
# Expansion is right-to-left. `layout.check` receives only the logical source
# replayed by `layout.choose`, not the original source layout.
layout_result = @layout.check @layout.choose false ("wrong")
  "first"
  "layout-ok"
