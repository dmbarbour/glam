...
| `eval/builtin_machine.rs`, `eval/comparison_machine.rs`, `eval/dict_machine.rs`, `eval/list_observation_machine.rs`, `eval/strategy_machine.rs` | Durable saturated-builtin owners; assertion, conditional-list-front, numeric, provenance, recursive comparison, dictionary key/path/operand, list observation, and strategy demand use resumable WHNF work while immediate validation, arithmetic, dictionary/list result construction, opaque-origin inspection, and post-access spark admission remain callback-free |
