...
| `core/managed/*_inventory.rs` | Source-backed durable-root, recursive-identity, containment, and active-RAII ownership gates; I9 classifies every production active `Drop`, while I10 closes deferred-callback, opaque/type-erasure, cache, and managed-backedge containment |
| `crates/glam-gc/` | Glam-owned typed-run tracing collector; each runtime value domain owns a no-auto heap, managed recursive identities are live, I6/I7 retain exact immutable-shell and persistent-container walks, I8 closes the net cycle matrix, I9 closes runtime-root lifecycle/retirement, and I10 closes external containment while production remains `NoAuto` |
