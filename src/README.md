...
| `crates/glam-gc/` | Glam-owned typed-run tracing collector; each runtime value domain owns a no-auto heap, managed recursive identities are live, I6/I7 retain exact immutable-shell and persistent-container walks, and I8C closes the net cycle matrix and retires core-specific compatibility identity adapters while production remains `NoAuto` |
