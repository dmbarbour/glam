...
| `core/managed/*_inventory.rs` | Source-backed durable-root, recursive-identity, containment, and active-RAII ownership gates; I9 composes these inventories and classifies every production active `Drop` implementation |
| `crates/glam-gc/` | Glam-owned typed-run tracing collector; each runtime value domain owns a no-auto heap, managed recursive identities are live, I6/I7 retain exact immutable-shell and persistent-container walks, I8 closes the net cycle matrix, and I9 closes runtime-root lifecycle/retirement while production remains `NoAuto` |
