...
pub(super) fn eval_compare_ordering_builtin(
    ...
    let ordering = compare_ordered_values(left, right, name)?;
...
pub(super) fn eval_compare_equality_builtin(
    ...
    let equal = equal_values(left, right, name)?;
...
fn compare_ordered_values(left: &Value, right: &Value, name: &str) -> Result<Ordering, EvalError> {
    ...
    match (left, right) {
        ...
        (Value::Binary(left), Value::List(right)) => {
            compare_lists_ordered(List::from_bytes(left), right, name)
        }
        (Value::List(left), Value::Binary(right)) => {
            compare_lists_ordered(left, List::from_bytes(right), name)
        }
        (Value::List(left), Value::List(right)) => compare_lists_ordered(left, right, name),
        (Value::Dict(left), Value::Dict(right)) => {
            ...
            compare_lists_ordered(left, right, name)
        }
        (Value::Builtin(_), _)
        | (_, Value::Builtin(_))
    ...
    loop {
        match (pop_list_front(&left)?, pop_list_front(&right)?) {
            ...
            (Some((left_head, left_tail)), Some((right_head, right_tail))) => {
                match compare_ordered_values(&left_head, &right_head, name)? {
                    Ordering::Equal => {
                        left = left_tail;
                        right = right_tail;
                    }
...
fn equal_values(left: &Value, right: &Value, name: &str) -> Result<bool, EvalError> {
    let left = force_value_shell(left)?;
    let right = force_value_shell(right)?;
    match (left, right) {
        ...
        (Value::Binary(left), Value::List(right)) => {
            equal_lists(List::from_bytes(left), right, name)
        }
        (Value::List(left), Value::Binary(right)) => {
            equal_lists(left, List::from_bytes(right), name)
        }
        (Value::List(left), Value::List(right)) => equal_lists(left, right, name),
        (Value::Dict(left), Value::Dict(right)) => equal_dicts(&left, &right, name),
...
fn equal_lists(mut left: List, mut right: List, name: &str) -> Result<bool, EvalError> {
    loop {
        match (pop_list_front(&left)?, pop_list_front(&right)?) {
            (None, None) => return Ok(true),
            (None, Some(_)) | (Some(_), None) => return Ok(false),
            (Some((left_head, left_tail)), Some((right_head, right_tail))) => {
                if !equal_values(&left_head, &right_head, name)? {
                    return Ok(false);
                }
                left = left_tail;
                right = right_tail;
...
fn equal_dicts(
    ...
    for (key, left_value) in left.iter() {
        ...
        if !equal_values(left_value, right_value, name)? {
            return Ok(false);
        }
    }
    ...
    for (key, right_value) in right.iter() {
        ...
        if !equal_values(&empty, right_value, name)? {
            return Ok(false);
        }
    }
