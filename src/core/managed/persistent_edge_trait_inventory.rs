...
fn persistent_edge_trait_occurrence_inventory_is_complete() {
    ...
    // W6G.1f.3g.2c keeps the production typed-edge total stable while moving
    // net work beneath the shared checkpoint. Its strengthened route-loss
    // fixture contributes one test-only owning-lazy root.
    // W6G.1f.3g.3a keeps dictionary progress as raw traced state beneath that
    // checkpoint; its forced collection fixture contributes one test-only
    // owning-lazy root.
    // W6G.1f.3g.3b similarly adds owning-lazy roots to the scalar and list
    // comparison route-loss fixtures while moving all production comparison
    ...
    // W6G.1f.3g.3e.3b adds the extraction schedule's owning-lazy root while
    // removing the final durable pattern and semantic-undefined root paths.
    // W6G.1f.3g.3f adds four test-only owning-lazy roots for the forced
    // diagnostic-context, collection, metadata, and reflection schedules;
    // production annotation state remains raw beneath the traced checkpoint.
    // W6G.1f.3g.4a adds rooted effect-call and effect-map schedules, while a
    // host semantic thunk captures its function through a reviewed root
    // instead of retaining an untraced raw managed value.
    // W6G.1f.3g.4b adds three test-only owning-lazy roots for object local-name,
    // dictionary-definition, and plain-dictionary conversion schedules. All
    // production object progress remains raw beneath the traced checkpoint.
    // W6G.1f.3g.4c adds three test-only owning-lazy roots for ordinary
    // extension, composed definitions, and recursive override. Production
    ...
    // PNC4R-002 adds the corresponding count and state promise roots for the
    // forced copy-operand handoff; fixture payload roots remain outside this
    // managed-edge inventory.
    ...
    // W6G.1e.3b.0 adds one test-only promise-root creation in the forced
    // child-before-wait scheduler fixture. Production edges are unchanged.
    // W6G.1e.2b renames two lazy/deferred scheduler fixtures to state their
    // causal-selection contract. A separate lazy-source fixture now roots its
    // promise across a forced collection, adding one test-only root creation.
    assert_eq!(
        occurrence_fingerprint(actual),
        7_373_437_538_390_803_908,
...
fn persistent_edge_inventory_classifications_are_closed() {
    ...
    assert_eq!(
        ...
        BTreeMap::from([
            ...
            ((SourceScope::Test, EdgeSurface::Typed), 622),
