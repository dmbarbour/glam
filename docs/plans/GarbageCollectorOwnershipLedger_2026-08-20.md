...
| `ManagedCoreNetCell` (introduced in I5) | One managed outer allocation owning the semantic mutex, revisions, normalization/subscriber state, and ordinary `RuntimeNet<CoreSpecialization>` containers. | One net mutex is the exclusive graph mutation boundary. No callback or destruction while locked. Exact trace uses `try_lock` under exclusive collection; passive managed destruction drops only ordinary Rust resources. GCI5R-002D's `RuntimeNetMutationGateway` observes the borrowed pre/post graph without reacquiring this mutex. No per-agent GC allocation. |
