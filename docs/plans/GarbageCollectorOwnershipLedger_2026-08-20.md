...
| `LazyValue`, `ManagedLazyCell`, `LazySource` | 8/8, 144/8, 40/8 | GCI5R-003F completed the façade cutover: `LazyValue` is one managed edge with no observer or copied metadata. I8A.0's smaller net edge reduced the containing lazy cell without changing `LazySource`'s maximum variant. Production observation requires bounded runtime access, while `ManagedLazyRoot` owns terminal publication and the durable scheduler/cycle record. |
...
inline and stores every other value in this registered node. I5D moved lazy,
promise, and core-net identities into exact managed cells and activated the
transitive compatibility walk to those leaves. Remaining aggregate payloads
retain compatibility Rust ownership after the I6/I7 audits. I8 completed the
final net-specific reconciliation and replaced the whole-net mutation bridge
with exact per-edit deltas.
...
| `List` | Persistent list nodes; value chunks and lazy/promise thunks are outgoing edges. | Immutable/persistent and thread-safe; may escape all evaluator calls. | I7 retained I4D's non-forcing logical visitor after an exhaustive representation audit and added the missing production list-thunk backedge reclamation fixture. Repeated shared-spine occurrences remain deliberate logical visits; bytes are leaves. No post-publication mutation. |
| `Dict` | `RedBlackTreeMapSync<Key, Value>`; every mapped value is an edge. Keys contain no live `Value` after conversion. | Immutable/persistent and thread-safe; may be runtime-global. | I7 retained I4D's mapped-value visitor after auditing empty/versioned maps and every recursive key form. I5B continues to compose it into the transitive managed-edge walk. No post-publication mutation. |
| `Builtin` | Static enum only. | Immutable leaf. | Leaf; I4. |
| `PartialBuiltin` | `BuiltinCall.arguments: Arc<[Value]>`. | Immutable, shared across threads. | I6A.1 retained the already-exact ordered argument visitor after direct and closed-cycle verification; no independent identity or mutation gateway exists. |
| `Function` | `FunctionValue -> NetValue -> CoreRuntimeNet`; the net facade carries one exact `ManagedCoreNetEdge`. | Immutable compatibility shell over the synchronized managed core-net cell; long-lived when reached from a root or another traced cell. | I6A.0 retained the immutable wrapper after proving `NetValue`, `FunctionCode`, and `FunctionValue` all project I5D's exact net identity without inspecting it. |
| `Net` | `NetValue -> CoreRuntimeNet`, whose facade carries one exact `ManagedCoreNetEdge`. | Immutable compatibility shell over synchronized managed runtime-net state. | I5D installed the managed outer-cell trace and bounded gateways. I8 replaced GCI5R-002D's whole-net correctness bridge with allocation-free exact per-edit deltas under the same net mutex and completed the final topology audit. |
...
| --- | --- | --- | --- |
| `EvaluationFailure` (`core.rs`) | Emission `Value` or cycle IDs; `Arc<[Value]>` contexts. | Immutable, thread-safe, retained in task/report ledgers. | I6C.1 retained the ordinary shell after exact ordered/no-forcing visitor and emission/context cycle tests; dependency-cycle IDs remain non-value leaves. `RuntimeFailureRoot` supplies shallow external roots only while a report owner lives. |
| `ManagedLazyCell` / `LazySource` / `ManagedLazyCheckpointEdge` (`core/managed/recursive_cells.rs`, `core.rs`, `eval/lazy_checkpoint.rs`) | Sources include fixpoint, explicit semantic computation, test-only semantic thunk, classified external host call, reflection computation, access arguments, application, builtin call, net construction/computation, and function call. Partial ordinary-WHNF and host-call work is retained as a concrete typed checkpoint edge; terminal `LazyResult` contains evaluated value or failure. Current x86-64 layout is 144/8 for the lazy cell; requested extent is 144 bytes, allocator discovery is exercised by the registered-root lifecycle fixture, and a separate checkpoint allocation exists only while partial work survives. | The producer mutex permits `Source -> Checkpoint` and reviewed `Checkpoint -> Checkpoint` handoffs; `result` is one-write and published before producer removal. Host invocation publishes `Invoking` before arbitrary Rust and only its installing route receives the transient invocation permit. Lock order is result check, producer mutex, result recheck; destruction occurs after unlock. | The exact I5D trace reports the terminal result when published, otherwise one stable source or checkpoint edge. W6G.1f.1 makes the checkpoint carrier field-opaque to core; W6G.1f.2a traces canonical WHNF state, and W6G.1f.3a.1 traces either the invoking host producer's declared captures or its published value/failure outcome. Private `allocate_managed_lazy` is bounded by `RuntimeValueAccess`; the exact first owner is a containing value root or `ManagedLazyRoot`. `ManagedLazyRoot::cache` remains the external terminal writer. Only the callback environment remains conservative external state; collection and tracing never invoke it. Destruction is passive under each `ManagedFamily` record. |
| `ManagedPromiseCell` (`core/managed/recursive_cells.rs`) | One successful ordinary `Value` edge or failure; immutable root-free producer routing record with a weak wait-state route; edge-free subscriptions. Current x86-64 layout is 104/8; requested extent is 104 bytes, and allocator discovery is exercised by the registered-root lifecycle fixture. | Assignment and producer are one-write. Coordinator mutation admission encloses task-owned publication; resolver publication uses its detached path. Notifications occur after publication. | The exact I5D trace visits assignment values/failures. Private `allocate_managed_promise` is bounded by `RuntimeValueAccess`; the exact first owner is a containing value root or `ManagedPromiseRoot`, and `fresh_managed_facades_survive_until_first_publication` forces the former gap. `ManagedPromiseRoot::{publish,publish_guarded,publish_detached}` are the external publication gateways and delegate to the private GCI5R-002C assignment transition; task/local owners and `PromiseResolver` hold those registered roots. I5E's `PromiseProducerPublication` temporarily owns roots removed by publication so their destruction occurs only after component locks and mutation admission are released. Outstanding external wait handles, not the managed promise, retain terminal wait results. Destruction is passive under its `ManagedFamily` record. |
| `MetadataCarrier` (`core.rs`) | One `Value`. | Immutable after construction. | I6B retained I4C's exact one-edge compatibility shell; no post-construction gateway or external semantic owner exists. |
| `BuiltinCall`, `Access`, `FunctionCall`, `LazyApplication` (`core.rs`) | Supplied function, arguments, and/or path leaf data. | Immutable `Arc` payloads; thread-safe. | I6A retained the exact ordered visitors and added closed lazy-application evidence. Function-call net identity already terminates at I5's managed net edge. |
| `FixpointComputation` (`core.rs`) | Function or object-instance `Value`. | Immutable. | I6A.2 retained the one-edge shell after direct and closed-cycle coverage of both variants. |
| `ReflectionComputation` and gate target (`core.rs`) | The managed-reachable computation directly owns immutable effect and optional target `Value` edges plus a passive external-owner handle. The registry owner caches only `Result<ReflectionTaskObservation, Arc<str>>`; the observation contains scalar identity/disposition and weak coordinator/wait authority. | The first observer alone receives a shared one-use activation permit with a temporary effect `RuntimeValueRoot`, profile, result policy, and bounded `EvalContext`. Activation transfers that ownership into the coordinator task machine. Strong task/wait handles are transient and terminal state owns result/failure roots only according to its disposition. | GCI5R-005/I6D.1 makes the compatibility adapter visit effect then target exactly. Registry drain is inert while a permit remains; dropping an unconsumed permit cancels reserved work, while activated work is owned and retired by the coordinator. Forced publication/handoff, terminal, backedge, and runtime-drop fixtures establish the complete chronology. |
| `SemanticComputation` / test-only `SemanticThunk` / `HostCallProducer` (`core.rs`) | Production semantic and host-call computation state store explicit ordered `Arc<[Value]>` captures. Capture-bearing semantic closures exist only in test builds. A host-call producer carries a mandatory source/capture record and an external-owner handle; immediately before invocation its traced values become one typed same-runtime `HostCallRootBundle`. | Immutable computation state/callbacks callable across workers. Host calls first move the producer into the lazy-owned `Invoking` checkpoint, root declared captures in a bounded access region, end that region, then run the opaque callback in a distinct mutator-free poll phase. | I4C reports ordinary semantic captures. I10A reports host-call captures through the managed lazy source, while W6G.1f.3a.1 continues that exact trace from the `Invoking` checkpoint until the rooted outcome becomes traced managed data. An interrupted invocation is never replayed. Arbitrary host closure environments remain deliberately conservative external owners and cannot be claimed traceable. |
| `FunctionCode`, `FunctionValue`, `NetValue` (`core.rs`) | Managed net edge plus scalar arity/capture state. | Immutable shells, synchronized net interior. | I6A.0 retained these scalar shells after proving they project I5D's exact identity without inspecting or reducing it; I8 completed their final net audit and retired their separate net-compatibility projections. |
| `CoreOperator` variants (`core_net.rs`) | Supplied values, function code, builtin call, applicable value; keys/path elements are leaves. | Immutable operator payloads stored in mutable nets. | The final I8 trace reports every supplied/direct value and nested function-code net identity without invoking the operator, and every semantic edit uses the exact mutation-gateway vocabulary. |
...
post-construction mutation gateway is required. I7 found no representation or
ownership reason to replace these spines or adapters. Its completed delta
audit retains the same logical edge policy and records duplicate shared-spine
visits as later profiling input rather than introducing a traversal cache.
...
| --- | --- | --- |
| `InteractionNet<CoreSpecialization>` template | Immutable node array; data nodes contain `Value`, operator nodes contain `CoreOperator`. | I5B composes the exact template visitor into the transitive walk; construction-local until published. I8's final source inventory found no post-I5 payload delta. |
| `CoreRuntimeNet` authority facade (introduced in I3D.3) | Private one-word core handle carrying exactly one `ManagedCoreNetEdge`, including stored cross-net source identities. | Every ordinary operation which locks and inspects or mutates semantic contents requires matching `RuntimeValueAccess`. Durable normalization, frontier, and source-copy owners retain root-only `ManagedCoreNetRoot`; the facade itself is an interior edge and cannot reopen or retain its runtime. |
| `ManagedCoreNetCell` (introduced in I5) | One managed outer allocation owning the semantic mutex, revisions, normalization/subscriber state, and ordinary `RuntimeNet<CoreSpecialization>` containers. Current x86-64 layout is 248/8; requested extent is 248 bytes, and allocator discovery is exercised by the registered-root lifecycle fixture. | One net mutex is the exclusive graph mutation boundary. No callback or destruction while locked. Exact trace uses `try_lock` under exclusive collection; passive managed destruction drops only ordinary Rust resources. Private `allocate_managed_core_net` is bounded by `RuntimeValueAccess`; the exact first owner is a containing value root or `ManagedCoreNetRoot`, and `fresh_managed_facades_survive_until_first_publication` forces the former gap. Exact per-edit mutation gateways observe transitions without reacquiring this mutex. No per-agent GC allocation. |
| Generic `RuntimeNetCell` / `SharedRuntimeNetState` plus `SharedRuntimeNet` adapter | External `Arc` owner retained for generic/non-core specializations and tests after the I5C.1 owner seam; the cell itself selects no owner. | Shares topology and synchronization behavior without depending on `glam_gc`; `RuntimeNetDisturbance` is an edge-free companion and the cell is not the production core-net owner after I5D. |
| `RuntimeNet`, `RuntimeEntry`, `RuntimeNode` | Data/operator values, ports, active-pair and cursor state. | I8A.1's compile-exhaustive owner inventory and the generic logical walk account for data, operators, retained source identities, and specialization stuck reasons without mutation or work claims. Free mutation remains under the owning net mutex and core authority facade. I8A.2 routes direct edits, conditional edits, active-pair steps, cursor claims, completion, and unwind restoration through exact per-edit deltas; no whole-net mutation visitor remains. |
...
| `NormalizationRequest`, `RequestRoot`, `Cursor`, `ActivePair`, `ResumeCursorDependency`, `NetDriverWorklist` (`eval/net.rs`) | Registered core-net roots plus edge-free interface, cursor, pair, and dependency snapshots. | Poll-orchestration owners project temporary core-net facades only through bounded evaluator access. Semantic call handling leaves the normalization access region before invoking evaluator logic. I3D.3 replaces manually paired call/cursor claim transitions with bracketed or lifetime-bound claims which cannot enter parked machine state. |
...
| `HostCallProducer` | Explicit recursive values plus an opaque external callback, but no inherited evaluator context. | I10A traces the values as managed lazy-source edges, converts them to one typed same-runtime root bundle before the callback, and requires a source/capture record. The arbitrary callback environment remains a conservative external owner. |
| `ModuleLoader`, `BinaryFileLoader`, `CompileDiagnosticEmitter` | Loader inputs may contain runtime roots; emitter input is one bounded value. | Loaders run outside inherited mutator access. Production loader constructors retain only weak runtime/execution routes; module-definition values reach them through `HostCallRootBundle`. The diagnostic emitter remains bounded to synchronous compilation. |
...
2. `ManagedLazyCell.result`, `ManagedPromiseCell.assignment`, reflection task installation,
   and promise producer installation are one-write edges. Their existing
   synchronization publishes fully initialized targets. GCI5R-002C routes the
   managed lazy-result and promise-assignment sites through the structural
   mutation gateway; the external reflection/producer records remain
   registered-root lifecycle surfaces rather than managed-object fields.
3. `ManagedLazyCell.source` is the only replaceable managed core-cell edge
   today. Its mutex guards removal; terminal result is published first.
   GCI5R-002C reports source removal and result installation as one coupled
   owner-qualified transition.
...
5. Interaction-net data/operator replacement is freely mutable under one net
   mutex. GCI5R-002D installs one `RuntimeNetMutationGateway` beneath every
   managed direct/conditional edit, active-pair reduction, cursor claim,
   completion, and unwind restoration. I8 replaced the policy-selected
   whole-net pre/post walk with allocation-free exact edge sets resolved under
   the same net mutex. Current STW `NoAuto` visits neither side;
   cursor/reduction helpers may not bypass the gateway.
...
| I4D non-forcing persistent collection adapters, logical work accounting, shared-spine behavior, and closed collection proof | `persistent_representation_to_visitor_inventory_is_complete`; `persistent_adapter_cycle_reclaims_in_isolated_heap`. |
| I4E non-reducing net/function payload classification, followed by I8C retirement of core-specific identity adapters | `core_operator_value_adapter_enumerates_every_value_payload`; `managed_core_net_trace_does_not_reduce_materialize_or_force`; `generic_runtime_net_payload_cycle_marks_exactly`; `managed_core_net_has_no_legacy_owner`. |
...
| I5 regional construction, exact first-owner chronology, and constructor-surface closure | `recursive_cell_gateways_are_private_and_complete`; `regional_constructor_inventory_detects_unreviewed_escape_shapes`; `regional_constructor_callers_are_exact_and_classified`; `fresh_managed_facades_survive_until_first_publication`; `regional_value_publication_retains_only_the_returned_managed_graph`; `early_regional_return_leaves_partial_managed_graph_collectible`; `evaluator_step_guards_fresh_managed_results_until_containing_root_publication`; `promise_settlement_releases_task_and_local_owner_roots`; `prepared_copy_source_is_an_exact_temporary_net_owner`. |
| I6D.1 reflection computation semantic edges, edge-free stable observation, one-use activation transfer, terminal ownership, and passive-cycle closure | `production_reflection_result_edges_do_not_need_an_external_root`; `production_reflection_gate_target_backedge_reclaims_without_an_external_root`; `latent_source_meta_refl_cycle_reclaims_with_its_module`; `reflection_gate_observer_and_activation_orderings_are_forced`; `reflection_handoff_transfers_effect_root_after_source_owner_retirement`; `terminal_wait_dispositions_retain_only_their_documented_runtime_roots`; `completed_reflection_owner_does_not_retain_its_value_domain`; `external_owner_recursive_backedges_are_explicitly_classified`. |
| I6 immutable function/application/fixpoint/metadata/failure/net-construction shells and durable failure-root retirement | `core_operator_value_adapter_enumerates_every_value_payload`; `argument_and_application_visitors_enumerate_exact_edges`; `compatibility_recursive_payload_visitors_enumerate_exact_edges`; `managed_cycle_through_object_fixpoint_is_traced_and_reclaimed`; `managed_cycle_through_lazy_application_is_traced_and_reclaimed`; `managed_cycle_through_net_construction_is_traced_and_reclaimed`; `managed_promise_cycle_through_metadata_is_traced_and_reclaimed`; both `managed_promise_cycle_through_failure_*` fixtures; `runtime_failure_root_alone_retains_and_releases_its_managed_values`. |
...
1. I10A closed the managed-to-external callback backedge: deferred import
   ...
   production loader routes into the assembler/runtime are weak. Arbitrary
   embedding callback environments remain opaque conservative Rust owners,
   but no production managed node hides its own recursive values there.
2. I5 atomically replaced lazy, promise, and production core-net recursive
   identities, installed one transitive compatibility walk and exact
   synchronized net trace, and closed GCI5R-001's regional first-owner
   chronology with a classified constructor inventory and forced-boundary
...
3. I10B selected and closed external-only opaque storage: the four-family
   ...
   I10C closed passive-handle destruction, active external retirement,
   domain-teardown fallback, panic suffix recovery, and conservative live-root
   retention. No managed opaque family or scoped managed downcast remains.
...
Gate G2 passed on 2026-09-11. I11A independently composed the authoritative
source inventories, added a stable-ledger completeness latch for the four
production managed families, repeated their isolated reclamation fixtures,
and reran the collector layout/class suite. The certification is recorded in
