...
| `LazyValue`, `ManagedLazyCell`, `LazySource` | 24/8, 160/8, 40/8 | GCI5R-003B removed duplicated façade identity metadata. Production observation now requires bounded runtime access; the façade temporarily retains its exact edge and weak domain observer until GCI5R-003F. |
| `PromisedValue`, `ManagedPromiseCell` | 24/8, 120/8 | GCI5R-003B/C removed duplicated façade metadata and the cell observer. GCI5R-003D moved edge-free terminal/subscription and producer routing into shared registered-root sidecars so coordinator traversal never opens the managed semantic payload; the façade observer remains only until GCI5R-003F. |
