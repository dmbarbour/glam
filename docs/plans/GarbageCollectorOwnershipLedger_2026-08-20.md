...
| `LazyValue`, `ManagedLazyCell`, `LazySource` | 8/8, 160/8, 40/8 | GCI5R-003F completed the façade cutover: `LazyValue` is one managed edge with no observer or copied metadata. Production observation requires bounded runtime access, while `ManagedLazyRoot` owns terminal publication and the durable scheduler/cycle record. |
...
| `ManagedLazyRoot`, `ManagedPromiseRoot`, `ManagedCoreNetRoot` | 32/8, 40/8, 24/8 | GCI5R-008 removed each separately cached managed edge; GCI5R-003F then removed lazy/promise root observers and the promise root's unused label. Lazy ID/label and promise ID copies have reviewed scheduler/diagnostic roles. Each root projects its exact edge only through matching bounded runtime access. These are measured implementation layouts, not ABI. |
| `CorePreparedCopySource`, `CoreFrontierObservation`, `NormalizationRequest` | 32/8, 48/8, 32/8 | GCI5R-008 replaced wrapper-hidden semantic net facades with one registered source/root plus edge-free operation snapshots. The generic facade is reconstructed only at its bounded consumption point. |
...
| `CoreRuntimeNet`, `ManagedCoreNetCell` | 24/8, 248/8 | I5D replaced the production core owner with the managed cell. Unlike the final one-pointer lazy/promise facades, `CoreRuntimeNet` temporarily retains weak value-domain provenance used by stored source identities and self-rooting/test helpers. GCI5R-003G assigns coordinated observer retirement to I8A.0. |
...
retain compatibility Rust ownership after the I6/I7 audits; I8 owns only the
final net-specific reconciliation and exact mutation-delta work.
...
| `Net` | `NetValue -> CoreRuntimeNet`, whose facade carries one exact `ManagedCoreNetEdge`. | Immutable compatibility shell over synchronized managed runtime-net state. | I5D installed the managed outer-cell trace and bounded gateways. GCI5R-002D now routes every core writer through a borrowed-state pre/post transition under the same net mutex; I8 replaces this whole-net correctness bridge with exact high-volume deltas and performs the final audit. |
...
| `FunctionCode`, `FunctionValue`, `NetValue` (`core.rs`) | Managed net edge plus scalar arity/capture state. | Immutable shells, synchronized net interior. | I6A.0 retained these scalar shells after proving they project I5D's exact identity without inspecting or reducing it; I8 performs the final net audit. |
| `CoreOperator` variants (`core_net.rs`) | Supplied values, function code, builtin call, applicable value; keys/path elements are leaves. | Immutable operator payloads stored in mutable nets. | I5D's transitive trace reports every supplied/direct value and nested function-code net identity without invoking the operator. I8 reconciles the final mutation-gateway vocabulary. |
...
| `CoreRuntimeNet` authority facade (introduced in I3D.3) | Private core handle carrying an exact `ManagedCoreNetEdge` plus temporary weak value-domain provenance. | Every ordinary operation which locks and inspects or mutates semantic contents requires matching `RuntimeValueAccess`. Durable normalization, frontier, and source-copy owners retain `ManagedCoreNetRoot`; the facade itself is an interior edge. GCI5R-003G assigns removal of the observer, self-rooting surface, and observer-qualified stored-source reconstruction to I8A.0 so the net-wide change is atomic. |
...
| `RuntimeNet`, `RuntimeEntry`, `RuntimeNode` | Data/operator values, ports, active-pair and cursor state. | I4E's generic logical walk reports data, operators, retained source identities, and specialization stuck reasons without mutation or work claims. Free mutation remains under the owning net mutex and core authority facade. GCI5R-002D routes direct edits, conditional edits, active-pair steps, cursor claims, completion, and unwind restoration through one managed gateway. Its current whole-net visitor is a correctness bridge; I8 replaces it with exact per-edit deltas and performs the final audit. |
...
5. Interaction-net data/operator replacement is freely mutable under one net
   ...
   completion, and unwind restoration. Its policy-selected whole-net pre/post
   walk is a correctness bridge and costs no traversal under current STW
   `NoAuto`; I8 replaces it with exact high-volume deltas. Cursor/reduction
   helpers may not bypass the gateway.
...
| I6 immutable function/application/fixpoint/metadata/failure/net-construction shells and durable failure-root retirement | `core_operator_adapter_enumerates_every_value_and_net_payload`; `argument_and_application_visitors_enumerate_exact_edges`; `compatibility_recursive_payload_visitors_enumerate_exact_edges`; `managed_cycle_through_object_fixpoint_is_traced_and_reclaimed`; `managed_cycle_through_lazy_application_is_traced_and_reclaimed`; `managed_cycle_through_net_construction_is_traced_and_reclaimed`; `managed_promise_cycle_through_metadata_is_traced_and_reclaimed`; both `managed_promise_cycle_through_failure_*` fixtures; `runtime_failure_root_alone_retains_and_releases_its_managed_values`. |
...
2. I5 atomically replaced lazy, promise, and production core-net recursive
   ...
   evidence. GCI5R-002 now supplies owner-qualified lazy, promise, and core-net
   transition gateways. I8 still performs the final post-cutover net audit and
   replaces the whole-net correctness bridge with exact high-volume deltas.
