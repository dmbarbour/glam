...
| `ManagedLazyCell` / `LazySource` (`core/managed/recursive_cells.rs`, `core.rs`) | Sources include fixpoint, explicit semantic computation, test-only semantic thunk, classified external host call, reflection computation, access arguments, application, builtin call, net construction/computation, and function call. Terminal `LazyResult` contains evaluated value or failure. Current x86-64 layout is 144/8; requested extent is 144 bytes, and allocator discovery is exercised by the registered-root lifecycle fixture. | `source` is replaceable only under its mutex; `result` is one-write and published before source removal. Lock order is result check, source mutex, result recheck; destruction occurs after unlock. | The exact I5D trace reports the terminal result when published, otherwise one stable source snapshot; it never invokes a producer. Private `allocate_managed_lazy` is bounded by `RuntimeValueAccess`; the exact first owner is a containing value root or `ManagedLazyRoot`, and `fresh_managed_facades_survive_until_first_publication` forces the former gap. `ManagedLazyRoot::cache` is the external terminal writer; its private access transition implements the GCI5R-002C source/result mutation. I10A makes every host-call semantic capture an ordinary traced source edge; only the callback environment remains conservative external state. Destruction is passive under its `ManagedFamily` record. |
