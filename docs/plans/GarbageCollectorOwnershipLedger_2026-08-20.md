...
| Diagnostics | `Diagnostic`, `DiagnosticEvent`, `DiagnosticBusState`, `DiagnosticBusInner`, `DiagnosticBus`, `DiagnosticIngressInner`, `DiagnosticIngress`, `DiagnosticSubscription`, `DiagnosticSubscriptionInner`, `Error`, `ReasoningFailure` | R, closed as compatibility roots by I4F.1e. Buses/callbacks are external root owners; events retain emission and origin public roots until event retirement. Failure records retain their existing root-shaped errors. Weak ingress/subscription back-references remain non-owning, and subscriber callbacks run outside locks and mutation admission. I4F.2c registers the surfaces; I9 audits them. |
| Runtime events | `RuntimeInputRecord`, `RuntimeOutputIntent`, `RuntimeDeliveryRecord`, `RuntimeEventSnapshot`, `RuntimeEventJournal`, `RuntimePreparedInput`, `RuntimeDeliveryTicket`, `RuntimeDiagnosticRoute` | R, closed as compatibility roots by I4F.1e. Persistent input snapshots and identified queued/running deliveries retain `RuntimeValueRoot` across threads and settlement. Input conversion precedes admission; output callbacks receive a retained public root after locks. Weak endpoint ownership remains non-owning. I4F.2c registers the surfaces; I9 audits them. |
