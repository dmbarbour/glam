...
| `RuntimeNet`, `RuntimeEntry`, `RuntimeNode` | Data/operator values, ports, active-pair and cursor state. | I8A.1's compile-exhaustive owner inventory and the generic logical walk account for data, operators, retained source identities, and specialization stuck reasons without mutation or work claims. Free mutation remains under the owning net mutex and core authority facade. GCI5R-002D routes direct edits, conditional edits, active-pair steps, cursor claims, completion, and unwind restoration through one managed gateway. Its current whole-net visitor is a correctness bridge; I8A.2 replaces it with exact per-edit deltas. |
