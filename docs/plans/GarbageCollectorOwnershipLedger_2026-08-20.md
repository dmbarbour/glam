...
3. `ManagedLazyCell.source` is the only replaceable managed core-cell edge
   today. Its mutex guards removal; terminal result is published first. I5D
   routes result installation through the bounded gateway and treats source
   removal as ordinary edge deletion.
...
2. I5 atomically replaced lazy, promise, and production core-net recursive
   ...
   evidence. GCI5R-002's value-installing mutation gateway remains open; I8
   performs the final post-cutover net audit and high-volume gateway work.
