...
| `MetadataCarrier`, `OpaqueValue` | 8/8, 16/8 | I6B audits whether metadata needs a managed identity or retains its exact one-edge compatibility shell; opaque remains a checked boundary. |
| `NetValue`, `FunctionCode`, `FunctionValue` | 8/8, 24/8, 16/8 | Net identity integration completed in I5; I6A.0 audits the remaining immutable scalar wrappers and defaults to retaining them. |
...
retain compatibility Rust ownership until I6-I8 replace their individual
structure steps.
...
| `PartialBuiltin` | `BuiltinCall.arguments: Arc<[Value]>`. | Immutable, shared across threads. | I4C compatibility visitor reports every supplied argument in order; I6A.1 audits this already-exact shell and converts it only for an independent representation benefit. |
| `Function` | `FunctionValue -> NetValue -> CoreRuntimeNet`; the net facade carries one exact `ManagedCoreNetEdge`. | Immutable compatibility shell over the synchronized managed core-net cell; long-lived when reached from a root or another traced cell. | I4E reports the stage-net identity without inspecting it; I5D made that identity an exact managed edge, including function-code nets nested in operators. I6A.0 audits and normally retains the immutable wrapper. |
...
| --- | --- | --- | --- |
| `EvaluationFailure` (`core.rs`) | Emission `Value` or cycle IDs; `Arc<[Value]>` contexts. | Immutable, thread-safe, retained in task/report ledgers. | I4C reports an emission followed by ordered contexts; dependency-cycle IDs are non-value leaves. I6C.1 audits this exact visitor and permits the ordinary Rust shell to remain. |
...
| `MetadataCarrier` (`core.rs`) | One `Value`. | Immutable after construction. | I4C reports exactly one edge; no post-construction gateway. I6B audits identity and the exact compatibility visitor before any optional conversion. |
| `BuiltinCall`, `Access`, `FunctionCall`, `LazyApplication` (`core.rs`) | Supplied function, arguments, and/or path leaf data. | Immutable `Arc` payloads; thread-safe. | I4C reports exact ordered semantic values. I5 migrated the function-call net identity and transitive walk; I6A audits the remaining immutable payloads and defaults to retaining them. |
| `FixpointComputation` (`core.rs`) | Function or object-instance `Value`. | Immutable. | I4C reports exactly one value; I6A.2 audits and normally retains this compatibility shell. |
...
| `FunctionCode`, `FunctionValue`, `NetValue` (`core.rs`) | Managed net edge plus scalar arity/capture state. | Immutable shells, synchronized net interior. | I5D made the net identity exact without inspecting or reducing it; I6A.0 audits the compatibility shells and I8 performs the final net audit. |
