...
| `LazyValue`, `ManagedLazyCell`, `LazySource` | 24/8, 160/8, 40/8 | GCI5R-003B removed duplicated façade identity metadata. Production observation requires bounded runtime access and GCI5R-003E moved terminal cache publication to `ManagedLazyRoot`; the façade temporarily retains its exact edge and weak domain observer until GCI5R-003F. |
| `PromisedValue`, `ManagedPromiseCell` | 24/8, 120/8 | GCI5R-003B/C removed duplicated façade metadata and the cell observer. GCI5R-003D moved edge-free terminal/subscription and producer routing into shared registered-root sidecars, and GCI5R-003E moved assignment publication to `ManagedPromiseRoot`; the façade observer remains only until GCI5R-003F. |
| `ManagedLazyRoot`, `ManagedPromiseRoot`, `ManagedCoreNetRoot` | 48/8, 72/8, 24/8 | GCI5R-008 removed the separately cached managed edge from each registered owner. Each root now projects its exact edge only through matching bounded runtime access. These are measured implementation layouts, not ABI. |
...
| `SharedRuntimeNet`, `ManagedCoreNetCell`, `SharedRuntimeNetState`, `RuntimeNetDisturbance` | 8/8 generic test facade, 248/8 managed core cell, 224/8 state, 8/8 companion | I5D replaced the production core owner with the managed cell. The generic `Arc` facade remains test-only; individual topology entries remain ordinary storage. |
...
| `FunctionCode`, `FunctionValue`, `NetValue` (`core.rs`) | Managed net edge plus scalar arity/capture state. | Immutable shells, synchronized net interior. | I5D made the net identity exact without inspecting or reducing it; I6 installs remaining function-shell visitors and I8 performs the final audit. |
