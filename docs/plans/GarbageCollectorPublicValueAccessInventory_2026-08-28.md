...
The 24 inventoried modules contain 192 compatibility occurrences after
...
| --- | ---: |
| borrowed `as_core()` projection | 71 |
| owned `into_core()` projection | 26 |
...
| Reflection lifecycle, machine, protocol, requests, search, store | 6 | 92 | I3D.1-I3D.4 phase boundaries; I4F.1 machine/store root surfaces |
...
- I4F.1d.3d.1 deliberately tripped both inventories while converting
  continuation, delimiter, and reset-frame payloads. Six raw declaration
  fields become six runtime roots; the added projections are confined to
  bounded continuation execution and reset-stack encoding.
- I4F.1d.3d.2 deliberately tripped both inventories while converting the
  shared fixpoint function. It becomes one runtime root, and exact root
  transfer into application work removes one owned projection.
- I4F.1d.3e deliberately tripped both inventories while closing decoded
  request handoff. Direct root transfer removes seven owned projections;
  rooting continuations at assignment also removes the obsolete fusion-state
  declaration and the prepared-drive phase-root bundle. Raw request and fused
  action values remain bounded to one evaluator step.
