...
- **Conditionals are effects**: booleans desugar to pass/fail effects (`.r ()` / `.fail`), `or`→raw ordered `.alt`, and `and`→`.seq`. Raw `or` may branch under a branching handler; parentheses only group it, while `.cut` commits to the first complete successful path. Pure `if`/`match` place the whole generated choice tree under one compiler-provided stateless `.cut`; `try`/`try_match` use an equivalent cut in the host environment (backtracking with state access).
