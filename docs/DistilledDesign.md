...
- **Annotations**: `anno Annotation Term` — never observable in evaluation (though may block it: divergence is unobservable). Uses: caching, sparks/parallelism, acceleration (substitute reference function with built-in, e.g. hardware emulators), `'error`/`'TBD`/`'deprecated`, scope-unique atoms. `anno 'meta_init ()` creates a sealed carrier and `anno meta_pure:UpdateFn Carriers` persistently transforms its hidden Glam metadata; ordinary evaluation cannot inspect it, while reflection may use `.meta.inspect`. This is a one-way evaluation-to-reflection boundary for traces and provenance, not an automatic operational log. Assembler warns on unrecognized annotations.
