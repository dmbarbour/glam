...
- **Objects** = dicts containing `spec:{name, defs, deps}`. `defs` is conceptually a mixin `\prior instance -> prior with ...`; `deps` lists parent specs; multiple inheritance via linearization (C3), deduplicating by `spec.name` (globally unique via `abstract_global_path` for toplevel declarations). Anonymous objects (`object _`) skip deduplication and apply before named parents. Plain dicts are not objects; `object_from_dict` explicitly converts one into an anonymous object.
