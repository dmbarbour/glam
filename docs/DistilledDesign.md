...
- **Objects** = dicts containing `spec:{name, defs, deps}`. `defs` is conceptually a mixin `\prior instance -> prior with ...`; `deps` lists parent specs; multiple inheritance via linearization (C3), deduplicating by `spec.name` and rejecting one name for distinct spec identities (globally unique via `abstract_global_path` for toplevel declarations). `abstract object` retains only the specification, while ordinary objects instantiate its definitions; `extend abstract` composes definitions but keeps the result abstract. Anonymous objects (`object _`) skip deduplication and apply before named parents. Plain dicts are not objects; `object_from_dict` explicitly converts one into an anonymous object.
...
- **Conditionals are effects**: booleans desugar to pass/fail effects (`.r ()` / `.fail`), `or`→raw ordered `.alt`, and `and`→`.seq`. Raw `or` may branch under a branching handler; parentheses only group it, while `.cut` commits to the first complete successful path. Pure `if`/`match` place the whole generated choice tree under one compiler-provided stateless `.cut`; `try`/`try_match` use an equivalent cut in the host environment (backtracking with state access). The open forms omit that root cut: `match*` collects the search as a lazy list, while `try_match*` exposes it to the ambient handler.
- **Annotations**: `anno Annotation Term` — never observable in evaluation (though may block it: divergence is unobservable). Uses: caching, sparks/parallelism, acceleration (substitute reference function with built-in, e.g. hardware emulators), `'error`/`'TBD`/`'deprecated`, scope-unique atoms. `anno 'meta_init ()` creates a sealed carrier; `meta_pure:UpdateFn` transforms hidden Glam metadata purely, while `meta_refl:EffectfulUpdate` transforms it through a lazily demanded reflection task. Ordinary evaluation can route but never inspect the result; reflection may use `.meta.inspect`. This is a sealed trace/provenance channel, not an automatic operational log. Assembler warns on unrecognized annotations.
...
CLI: `-f file` / `-s.ext script` / `-- args` build an anonymous assembly module (earlier files override later); `--refl arg` supplies reflection-only arguments, `--manifest file` records secure hashes of local inputs, standalone `--check_manifest file [--quiet]` verifies them, and `--workers N` selects the shared background evaluator pool (`GLAM_WORKERS` supplies the default). Output: `asm.result` as `--binary` (default, to `--stdout`) or `--folder` (`-o dest`); `asm.file_meta` for permissions; atomic file replacement. Modes: `--batch` (default) or `-i` (runs `conf.ide`: TTY, TCP/socket, light GUI, limited file editing).
