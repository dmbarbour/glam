...
| `SemanticThunk` / `HostCallProducer` dispatch in `produce_lazy_source` in [`eval/value.rs`](../../src/eval/value.rs) | I3E.1 now restricts semantic thunks to `EvaluatorStepContext`. A separate host-call phase invokes a context-free producer outside evaluator authority, then validates and projects its rooted result in a bounded step. | Resolved by I3E.1: no arbitrary host callback remains hidden inside an evaluator scope. I4B/I10 still own closure-capture containment. |
| `CompileContext::{import_module, import_binary}` in [`compiler.rs`](../../src/compiler.rs) | I3E.1 now implements imports as deferred host calls whose module/binary loaders return compilation-runtime roots. Source artifacts retain digest identity and local files retain stable-read/final-verification checks. | Resolved operationally by I3E.1. Imports remain distinct from reflection in policy and provenance; I4B/I10 still audit host captures. |
...
6. import loaders become deterministic deferred host calls using analogous gate
